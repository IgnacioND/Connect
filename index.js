...
function paintHTML(objectID) {
  ...
  buttons.innerHTML += `<div class="buttonP" id="${objectID}"></div>\n`;
...
function changeTurn(turnEntry) {
  ...
  if (played === true) {
    played = false;
    deleteClassButtons(turns[currentTurn]);
    currentTurn = (currentTurn + 1) % 2;
    turns[currentTurn];
    assignClassButtons(turns[currentTurn]);
    return;
  } else {
    turn = turnEntry;
    return;
...
function clearButtons() {
  ...
  buttonsArr.forEach((element) => {
    element.classList.add("buttonP");
    element.classList.remove("buttonOff");
  });
}
function deleteClassButtons(turn) {
  let buttonsArr = Array.from(document.getElementsByClassName(`button${turn}`));
  buttonsArr.forEach((element) => {
    element.classList.remove(`button${turn}`);
    element.classList.add(`buttonToChange`);
  });
}
function assignClassButtons(turn) {
  let buttonsArr = Array.from(
    document.getElementsByClassName(`buttonToChange`)
  );
  buttonsArr.forEach((element) => {
    element.classList.remove(`buttonToChange`);
    element.classList.add(`button${turn}`);
  });
}
...
function chipPrint(elementID, turn) {
  ...
  clickedButton.classList.remove("buttonP");
  clickedButton.classList.remove("buttonC");
